...
export default defineEventHandler(async event => {
    ...
    const subject = locale === 'en'
        ? `${personalInfo?.name || 'Admin'} - Temporary password for ${user.email}`
        : `${personalInfo?.name || 'Admin'} - Mot de passe temporaire pour ${user.email}`
    ...
    const mailOptions = {
        from: `"${config.adminName || 'Admin'}" <${config.smtpUser}>`, // sender = SMTP
        to: config.smtpUser, // mail arrives to the SMTP account itself
        subject,
        text,
        html: renderTempPasswordEmail({ userEmail: user.email, tempPassword, message, adminName: config.adminName || 'Admin', locale, config })
    }
