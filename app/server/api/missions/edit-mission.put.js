...
export default defineEventHandler(async event => {
  ...
  const { id, clientId, serviceId, title, date, duration, quantity, unitPrice, tjm, tvaApplicable } = body
  ...
  const monthConcerned = date.slice(0, 7)
  ...
  try {
    await db.sql`
      UPDATE dc_missions SET
        ...
        title = ${title || null},
        date = ${date},
        month_concerned = ${monthConcerned},
        duration = ${duration || 0},
        quantity = ${quantity || 1},
